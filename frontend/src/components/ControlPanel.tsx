...
export const ControlPanel: React.FC<ControlPanelProps> = ({
  ...
  const handleAddNode = () => {
    if (!newNodeId.trim() || !socket) return;
    
    socket.emit('register-node', newNodeId.trim());
    setNewNodeId('');
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
        ...
        <div className="space-y-4">
          <input
            ...
            placeholder="Node ID (e.g., node-1)"
            className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500"
            onKeyPress={(e) => e.key === 'Enter' && handleAddNode()}
          />
          <button
            onClick={handleAddNode}
            disabled={!newNodeId.trim() || !connected}
            className="w-full px-4 py-3 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 disabled:from-gray-600 disabled:to-gray-600 text-white font-medium rounded-lg transition-all duration-200 disabled:cursor-not-allowed"
          >
            Add Node
          </button>
        </div>
      ...
      <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
        ...
        <button
          ...
          disabled={!connected}
          className="w-full px-4 py-3 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 disabled:from-gray-600 disabled:to-gray-600 text-white font-medium rounded-lg transition-all duration-200 disabled:cursor-not-allowed"
        >
          Add 3 Sample Nodes
        </button>
      </div>
      ...
      <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
        ...
        <div className="space-y-4">
          <button
            ...
            disabled={!connected || nodes.filter(n => n.isAlive).length < 2}
          ...
          <button
            ...
            disabled={!logCommand.trim() || !connected}
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <button
                    ...
                    disabled={!connected || !node.isAlive}
